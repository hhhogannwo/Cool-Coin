...
CoolCoin version 24.0.1 is now available from:

  <https://www.Cool-Coin.orgbin/bitcoin-core-24.0.1/>
...
installer (on Windows) or just copy over `/Applications/CoolCoin-Qt` (on macOS)
...
This version of CoolCoin adds a new `mempoolfullrbf` configuration
...
Some CoolCoin services today expect that the first version of an
...
The CoolCoin Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [CoolCoin 0.12.0][] (February 2016) with the
...
default for years, and several contributors to CoolCoin are
...
[CoolCoin 0.12.0]: https://www.Cool-Coin.orgen/releases/0.12.0/#opt-in-replace-by-fee-transactions
...
- To help prevent fingerprinting transactions created by the CoolCoin wallet, change output
...
[documentation](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/managing-wallets.md#migrating-legacy-wallets-to-descriptor-wallets).
...
- Also, the interaction between GUI settings and `coolcoin.conf` settings is
simplified. Settings from `coolcoin.conf` are now displayed normally in the GUI
settings dialog, instead of in a separate warning message ("Options set in this
dialog are overridden by the configuration file: -setting=value"). And these
settings can now be edited because `settings.json` values take precedence over
`coolcoin.conf` values. (#15936)
