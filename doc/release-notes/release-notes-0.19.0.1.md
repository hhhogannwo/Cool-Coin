CoolCoin version 0.19.0.1 is now available from:

  <https://www.Cool-Coin.orgbin/bitcoin-core-0.19.0.1/>
...
installer (on Windows) or just copy over `/Applications/CoolCoin-Qt` (on Mac)
or `bitcoind`/`bitcoin-qt` (on Linux).

Upgrading directly from a version of CoolCoin that has reached its EOL is
possible, but might take some time if the datadir needs to be migrated.  Old
wallet versions of CoolCoin are generally supported.

Compatibility
==============

CoolCoin is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use CoolCoin on unsupported systems.
...
to open the CoolCoin .dmg. This is due to new signing requirements
imposed by Apple, which the CoolCoin project does not yet adhere too.
...
- [Reduce memory](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running CoolCoin on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of CoolCoin
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile CoolCoin with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in CoolCoin 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of CoolCoin. (#14054)

- To eliminate well-known denial-of-service vectors in CoolCoin,
  ...
  This prevents CoolCoin from sending the BIP111 NODE_BLOOM service
  ...
  of CoolCoin and nodes that have manually enabled BIP37 support,
...
- By default, CoolCoin will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how CoolCoin determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  CoolCoin would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, CoolCoin will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 CoolCoin address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in CoolCoin v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@CoolCoin AEAD (jonasschnelli)
...
- #15773 Add CoolCoinTestFramework::sync_* methods (MarcoFalke)
...
- #15896 feature_filelock, interface_bitcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding CoolCoin (luke-jr)
...
- #15513 add "sections" info to example coolcoin.conf (fanquake)
...
- #16080 build/doc: update bitcoin-config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in coolcoin.conf (fanquake)
...
- #16621 add default coolcoin.conf locations (GChuf)
...
- #16812 Fix whitespace errs in .md files, coolcoin.conf, and Info.plist.in (ch4ot1c)
