...
CoolCoin version 26.0 is now available from:

  <https://www.Cool-Coin.orgbin/bitcoin-core-26.0/>
...
using the Linux kernel, macOS 11.0+, and Windows 7 and newer.  CoolCoin
...
- `bitcoind` and `bitcoin-qt` will now raise an error on startup
 if a datadir that is being used contains a coolcoin.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a coolcoin.conf file. The error message is just a diagnostic intended
 ...
 the coolcoin.conf contained in it. (#27302)
...
- Wallet loading has changed in this release. Wallets with some corrupted records that could be
  ...
  load the wallet in a previous version of CoolCoin and import the data into a new wallet.
...
- The usage of hybrid public keys in output descriptors has been removed. Hybrid
  ...
  (as specified in BIP380 and documented in doc/descriptors.md). CoolCoin would
