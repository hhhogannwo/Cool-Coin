...
CoolCoin version 0.21.0 is now available from:

  <https://www.Cool-Coin.orgbin/bitcoin-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by CoolCoin, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. CoolCoin will no longer
...
- The `startupnotify` option is used to specify a command to
  execute when CoolCoin has finished with its startup
  sequence. (#15367)
...
CoolCoin will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`coolcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
CoolCoin will now just log warnings instead of creating new wallets with
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/issues).
...
Users who already have a CoolCoin wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of CoolCoin.
...
The change to using descriptors changes the default derivation paths used by CoolCoin
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time CoolCoin is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `coolcoin.conf`
...
- #18739 Document how to fuzz CoolCoin using Honggfuzz (practicalswift)
