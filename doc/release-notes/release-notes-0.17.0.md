CoolCoin version 0.17.0 is now available from:

  <https://www.Cool-Coin.orgbin/bitcoin-core-0.17.0/>
...
CoolCoin is extensively tested on multiple operating systems using
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `coolcoin.conf` file, not inside included files or from
  ...
  as coolcoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the coolcoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
BIP 174 Partially Signed CoolCoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for CoolCoin transactions that are not fully signed
...
Overall, the construction of a fully signed CoolCoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid CoolCoin transaction (in network format)
...
- #13528 `b877c39` Move CoolCoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use CoolCoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to coolcoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use CoolCoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use CoolCoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use CoolCoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for CoolCoinTestFramework (WillAyd)
