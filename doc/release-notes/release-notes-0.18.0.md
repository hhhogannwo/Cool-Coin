CoolCoin version 0.18.0 is now available from:

  <https://www.Cool-Coin.orgbin/bitcoin-core-0.18.0/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/issues>

To receive security and update notifications, please subscribe to:

  <https://www.Cool-Coin.orgen/list/announcements/join/>
...
`/Applications/CoolCoin-Qt` (on Mac) or `bitcoind`/`bitcoin-qt` (on
Linux).
...
CoolCoin is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use CoolCoin on unsupported systems.

CoolCoin should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, CoolCoin does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://www.Cool-Coin.orgen/contact/) the CoolCoin
...
`~bitcoin/.bitcoin`. This change makes CoolCoin more consistent with
...
`datadir` option in `/etc/bitcoin/coolcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/bitcoin/coolcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/coolcoin.conf.md)
  about the `coolcoin.conf` file describes how to use it to configure
  CoolCoin.

- A new document introduces CoolCoin's BIP174 [Partially-Signed
  CoolCoin Transactions
  (PSBT)](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent CoolCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  CoolCoin to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when CoolCoin knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of CoolCoin, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- A new `bitcoin-wallet` tool is now distributed alongside CoolCoin
...
This section describes planned changes to CoolCoin that may affect
other CoolCoin software and services.

- Since version 0.16.0, CoolCoin’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with CoolCoin 0.20 (expected about
  a year after 0.18), CoolCoin will default to native segwit
  ...
  to bech32 addresses, and if the CoolCoin project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in CoolCoin 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any CoolCoin
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, CoolCoin
...
- A sub-project of CoolCoin now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with CoolCoin.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to CoolCoin's own implementation, although entropy
  gathered by CoolCoin is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves CoolCoin a
...
- On macOS, CoolCoin now opts out of application CPU throttling
...
- #11625 Add CoolCoinApplication & RPCConsole tests (ryanofsky)
...
- #11526 Visual Studio build configuration for CoolCoin (sipsorcery)
...
- #12775 Integration of property based testing into CoolCoin (Christewart)
...
- #14057 logging: Only log `using config file path_to_coolcoin.conf` message on startup if conf file exists (leishman)
...
- #14497 Add doc/coolcoin.conf.md (hebasto)
