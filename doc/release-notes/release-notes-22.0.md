...
CoolCoin version 22.0 is now available from:

  <https://www.Cool-Coin.orgbin/bitcoin-core-22.0/>
...
using the Linux kernel, macOS 10.14+, and Windows 7 and newer.  CoolCoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use CoolCoin on
unsupported systems.

From CoolCoin 22.0 onwards, macOS versions earlier than 10.14 are no longer supported.
...
-----------------------
- Added support for running CoolCoin as an
  [I2P (Invisible Internet Project)](https://en.wikipedia.org/wiki/I2P) service
  and connect to such services. See [i2p.md](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/22.x/doc/i2p.md) for details. (#20685)
- This release removes support for Tor version 2 hidden services in favor of Tor
  v3 only, as the Tor network [dropped support for Tor
  v2](https://blog.torproject.org/v2-deprecation-timeline) with the release of
  Tor version 0.4.6.  Henceforth, CoolCoin ignores Tor v2 addresses; it
...
- The list of banned hosts and networks (via `setban` RPC) is now saved on disk
  ...
  `banlist.json`. A future version of CoolCoin may completely ignore
  `banlist.dat`. (#20966)
...
- A new CLI `-addrinfo` command returns the number of addresses known to the
  ...
  like `-onlynet=<network>` or to upgrade to this release of CoolCoin 22.0
...
- External signers such as hardware wallets can now be used through the new RPC methods `enumeratesigners` and `displayaddress`. Support is also added to the `send` RPC call. This feature is experimental. See [external-signer.md](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/22.x/doc/external-signer.md) for details. (#16546)
...
- Taproot descriptors can be imported into the wallet only after activation has occurred on the network (e.g. mainnet, testnet, signet) in use. See [descriptors.md](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/22.x/doc/descriptors.md) for supported descriptors.
...
- bitcoin-core/gui#21 Improve pruning tooltip (fluffypony, CoolCoinErrorLog)
...
- bitcoin/bitcoin#20577 libconsensus: add missing error code description, fix NCoolCoin link (theStack)
...
- bitcoin/bitcoin#21384 doc: add signet to coolcoin.conf documentation (jonatack)
...
- bitcoin/bitcoin#21709 Update reduce-memory.md and coolcoin.conf -maxconnections info (jonatack)
...
- bitcoin/bitcoin#21818 Fixup -coinstatsindex help, update coolcoin.conf and files.md (jonatack)
...
- bitcoin/bitcoin#22080 add maxuploadtarget to coolcoin.conf example (jarolrod)
...
- bitcoin/bitcoin#22393 Added info to coolcoin.conf doc (bliotti)
