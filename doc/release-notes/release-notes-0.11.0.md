CoolCoin version 0.11.0 is now available from:
...
For example, add the following to `coolcoin.conf`:
...
Block pruning allows CoolCoin to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that CoolCoin will 
...
- #5729 `9c4a5a5` Allow unit changes for read-only CoolCoinAmountField
...
- #6116 `30dc3c1` [OSX] rename CoolCoin-Qt.app to CoolCoin-Core.app
