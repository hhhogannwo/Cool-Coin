...
CoolCoin version 0.20.0 is now available from:

  <https://www.Cool-Coin.orgbin/bitcoin-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  CoolCoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use CoolCoin on
unsupported systems.

From CoolCoin 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, CoolCoin does not yet change appearance
...
#### Removal of BIP61 reject network messages from CoolCoin

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since CoolCoin version 0.18.0.
...
- Testing or debugging of implementations of the CoolCoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of CoolCoin. CoolCoin logs debug messages
...
- OpenSSL is no longer used by CoolCoin.  (#17265)

- BIP70 support has been fully removed from CoolCoin. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/issues/16599),
  [PR #16702](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/pull/16702), and the
...
- All custom settings configured when CoolCoin starts are now
...
- The "Start CoolCoin on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://www.Cool-Coin.orgen/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple CoolCoin wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed CoolCoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoin-core/HWI) for
  signing.  Future versions of CoolCoin should support a GUI option
...
- CoolCoin's automatically-generated source code documentation is
...
- #17330 Add `shrinkdebugfile=0` to regtest coolcoin.conf (sdaftuar)
...
- #16983 Add detailed info about CoolCoin files (hebasto)
