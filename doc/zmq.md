...
The CoolCoin daemon can be configured to act as a trusted "border
...
The ZeroMQ feature in CoolCoin requires the ZeroMQ API >= 4.0.0
...
terminator). These options can also be provided in coolcoin.conf.
...
using. CoolCoind appends an up-counting sequence number to each
