...
Clone the CoolCoin repository to a directory. All build scripts and commands will run from this directory.
...
`sqlite3` is required to enable support for [descriptor wallets](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/doc/descriptors.md).
...
CoolCoin includes a GUI built with the cross-platform Qt Framework. To compile the GUI, we need to install `qt5`.
...
There are many ways to configure CoolCoin. Here is an example that
