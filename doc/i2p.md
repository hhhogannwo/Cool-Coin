# I2P support in CoolCoin

It is possible to run CoolCoin as an
...
## Run CoolCoin with an I2P router (proxy)

A running I2P router (proxy) is required with the [SAM](https://geti2p.net/en/docs/api/samv3)
application bridge enabled. The following routers are recommended for use with CoolCoin:
...
Once an I2P router with SAM enabled is up and running, use the following CoolCoin
...
I2P support was added to CoolCoin in version 22.0 and there may be fewer I2P
...
The first time CoolCoin connects to the I2P router, it automatically
...
`i2p_private_key` in the CoolCoin data directory.  The persistent I2P
...
## Fetching I2P-related information from CoolCoin

There are several ways to see your I2P address in CoolCoin if accepting
...
CoolCoin uses the [SAM v3.1](https://geti2p.net/en/docs/api/samv3) protocol
to connect to the I2P network. Any I2P router that supports it can be used.

## Ports in I2P and CoolCoin
...
To allow future upgrades to newer versions of SAM, CoolCoin sets its
listening port to 0 when listening for incoming I2P connections and advertises
its own I2P address with port 0. Furthermore, it will not attempt to connect to
I2P addresses with a non-zero port number because with SAM v3.1 the destination
port (`TO_PORT`) is always set to 0 and is not in the control of CoolCoin.
...
It's important that the nodes of a popular application like CoolCoin contribute
...
## Bundling I2P in a CoolCoin application

Please see the "General Guidance for Developers" section in https://geti2p.net/en/docs/api/samv3
if you are developing a downstream application that may be bundling I2P with CoolCoin.
