# User-space, Statically Defined Tracing (USDT) for CoolCoin

CoolCoin includes statically defined tracepoints to allow for more
...
**Note**: CoolCoin uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to CoolCoin
...
To list probes in CoolCoin, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in CoolCoin.
