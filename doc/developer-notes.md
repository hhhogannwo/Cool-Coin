...
- [Development guidelines](#development-guidelines)
    - [General CoolCoin](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
CoolCoin uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
CoolCoin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/blob/master/contrib/valgrind.supp))
...
CoolCoin can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/issues/12691)
...
However, in open source software such as CoolCoin, where everyone uses
...
pay attention to for reviewers of CoolCoin code.
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
  ...
  - *Rationale*: CoolCoin uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of CoolCoin, in which case
...
a CoolCoin subtree, so that they can be integrated quickly. Cosmetic changes
...
file descriptors, it will cause problems with CoolCoin's `select()` loop, because
...
compatibility between nodes. This happened in CoolCoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to CoolCoin's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of CoolCoin, the wallet was always
...
- Interface method names should be `lowerCamelCase` and standalone function names should be
  ...
  *Rationale*: consistency and friendliness to code generation tools.
  ...
  before in [#14635](https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/pull/14635).
