...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<CoolCoinUnit>("CoolCoinUnit");
#else
    qRegisterMetaType<CoolCoinUnit>("CoolCoinUnit");
#endif
}
...
CoolCoinApplication::CoolCoinApplication()
...
void CoolCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the CoolCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", CoolCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void CoolCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool CoolCoinApplication::createOptionsModel(bool resetSettings)
{
...
void CoolCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new CoolCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &CoolCoinGUI::quitRequested, this, &CoolCoinApplication::requestShutdown);
...
void CoolCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void CoolCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool CoolCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void CoolCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &CoolCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &CoolCoinApplication::handleRunawayException);
    connect(this, &CoolCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &CoolCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void CoolCoinApplication::parameterSetup()
{
...
void CoolCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void CoolCoinApplication::requestInitialize()
{
...
void CoolCoinApplication::requestShutdown()
{
...
void CoolCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &CoolCoinGUI::handlePaymentRequest);
            connect(window, &CoolCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void CoolCoinApplication::handleRunawayException(const QString &message)
{
...
void CoolCoinApplication::handleNonFatalException(const QString& message)
{
...
WId CoolCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool CoolCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", CoolCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6-7. Parse coolcoin.conf, determine network, switch to network specific
