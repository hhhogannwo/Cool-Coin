// Copyright (c) 2011-2022 The CoolCoin developers
...
const std::string CoolCoinGUI::DEFAULT_UIPLATFORM =
...
CoolCoinGUI::CoolCoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &CoolCoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &CoolCoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &CoolCoinGUI::showModalOverlay);
...
void CoolCoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a CoolCoin address"));
    ...
    connect(overviewAction, &QAction::triggered, this, &CoolCoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &CoolCoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &CoolCoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your CoolCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified CoolCoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed CoolCoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed CoolCoin Transaction from clipboard"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible CoolCoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &CoolCoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &CoolCoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &CoolCoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &CoolCoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &CoolCoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &CoolCoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &CoolCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &CoolCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &CoolCoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &CoolCoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &CoolCoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &CoolCoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &CoolCoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &CoolCoinGUI::showDebugWindow);
}

void CoolCoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void CoolCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &CoolCoinGUI::setCurrentWalletBySelectorIndex);
...
void CoolCoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &CoolCoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &CoolCoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &CoolCoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &CoolCoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void CoolCoinGUI::enableHistoryAction(bool privacy)
{
...
void CoolCoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &CoolCoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &CoolCoinGUI::removeWallet);
...
WalletController* CoolCoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void CoolCoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &CoolCoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &CoolCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &CoolCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &CoolCoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &CoolCoinGUI::incomingTransaction);
    connect(this, &CoolCoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void CoolCoinGUI::removeWallet(WalletModel* walletModel)
{
...
void CoolCoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void CoolCoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void CoolCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void CoolCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void CoolCoinGUI::createTrayIcon()
{
...
void CoolCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &CoolCoinGUI::toggleHidden);
...
void CoolCoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void CoolCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void CoolCoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void CoolCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void CoolCoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void CoolCoinGUI::openClicked()
{
...
void CoolCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void CoolCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void CoolCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void CoolCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void CoolCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void CoolCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void CoolCoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void CoolCoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to CoolCoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void CoolCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void CoolCoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void CoolCoinGUI::updateHeadersSyncProgressLabel()
{
...
void CoolCoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void CoolCoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &CoolCoinGUI::quitRequested);
...
void CoolCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void CoolCoinGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &CoolCoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void CoolCoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void CoolCoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &CoolCoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &CoolCoinGUI::show);
                e->ignore();
            }
        }
    }
...
void CoolCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void CoolCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void CoolCoinGUI::incomingTransaction(const QString& date, CoolCoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(CoolCoinUnits::formatWithUnit(unit, amount, true));
...
void CoolCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CoolCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool CoolCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool CoolCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void CoolCoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void CoolCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void CoolCoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void CoolCoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void CoolCoinGUI::updateWindowTitle()
{
...
void CoolCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void CoolCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CoolCoinGUI::detectShutdown()
{
...
void CoolCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void CoolCoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(CoolCoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = CoolCoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void CoolCoinGUI::subscribeToCoreSignals()
{
...
void CoolCoinGUI::unsubscribeFromCoreSignals()
{
...
bool CoolCoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<CoolCoinUnit> units = CoolCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const CoolCoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, CoolCoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const CoolCoinUnit u : CoolCoinUnits::availableUnits()) {
        menu->addAction(CoolCoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(CoolCoinUnit newUnits)
{
    setText(CoolCoinUnits::longName(newUnits));
}
