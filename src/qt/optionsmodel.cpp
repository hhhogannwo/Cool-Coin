...
/** Call node.updateRwSetting() with CoolCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because CoolCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/pull/24498.
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayCoolCoinUnit")) {
        settings.setValue("DisplayCoolCoinUnit", QVariant::fromValue(CoolCoinUnit::CC));
    }
    QVariant unit = settings.value("DisplayCoolCoinUnit");
    if (unit.canConvert<CoolCoinUnit>()) {
        m_display_bitcoin_unit = unit.value<CoolCoinUnit>();
    } else {
        m_display_bitcoin_unit = CoolCoinUnit::CC;
        settings.setValue("DisplayCoolCoinUnit", QVariant::fromValue(m_display_bitcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if coolcoin.conf value
...
{
    if (new_unit.isNull() || new_unit.value<CoolCoinUnit>() == m_display_bitcoin_unit) return;
    m_display_bitcoin_unit = new_unit.value<CoolCoinUnit>();
    QSettings settings;
    settings.setValue("DisplayCoolCoinUnit", QVariant::fromValue(m_display_bitcoin_unit));
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/hhhogannwo/Cool-Coin/Cool-Coin/pull/8273
